...
function speedStyle(
  ...
  if (ms < 250) return { bar: "bg-blue-400", text: "text-blue-500" };
...
function avgMs(results: ProbeResult[]): number | null {
  ...
  return Math.round(
    ok.reduce((sum, r) => sum + (r.ms as number), 0) / ok.length,
  );
}

function avgColor(ms: number | null): string {
  if (ms === null) return "text-red-500";
  if (ms < 250) return "text-blue-600";
...
function statusLabel(ms: number | null): { label: string; dot: string } {
  ...
  if (ms < 250) return { label: "good", dot: "bg-blue-400" };
        ...
        <div className="grid grid-cols-2 gap-4">
          {rooms.map((room) => {
            ...
            return (
              <div
                key={room.room}
                className="bg-zinc-100 rounded-2xl p-5 flex flex-col gap-5"
              >
                {/* Card header */}
                ...
                <div className="flex flex-col items-center py-1">
                  {avg !== null ? (
                    <>
                      <span
                        className={`text-5xl font-semibold tabular-nums leading-none ${avgColor(avg)}`}
                      >
                        {avg}
                      </span>
                      ...
                      return (
                        <div key={r.url} className="flex flex-col gap-1">
                          <div className="flex justify-between items-baseline text-xs">
                            ...
                            <span
                              className={`tabular-nums ml-1 shrink-0 font-medium ${s.text}`}
                            >
                              {r.ok ? `${r.ms}` : "—"}
                            </span>
                          </div>
